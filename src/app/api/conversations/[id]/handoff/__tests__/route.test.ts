...
function buildRequest(status = "handed_off"): NextRequest {
  return new NextRequest("https://example.com/api/conversations/x/handoff", {
    method: "POST",
...
describe("POST /api/conversations/[id]/handoff", () => {
  ...
  beforeAll(async () => {
    ...
    business = await createTestBusiness(owner.id, "conv-handoff");
    conversation = await createTestConversation(business.id, "3");
  });
